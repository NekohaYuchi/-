# Dash 儀表板 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 將 `system.py`（Gradio）改寫為 `system_dash.py`（Dash），實現側邊欄導覽、深海藍主題、三頁面設計，並修復啟動空白問題。

**Architecture:** 單一檔案 `system_dash.py`，控制面板與導覽常駐於左側邊欄（永遠在 DOM 中）。三個頁面內容全部渲染於 DOM，以 CSS `display` 切換顯示，避免 Dash callback 的 component 不在 DOM 的問題。`prevent_initial_call=False` 使 callback 在頁面載入時即觸發，修復啟動空白。

**Tech Stack:** Dash 2.x, dash-bootstrap-components 1.x, Plotly, pandas, numpy, joblib, scikit-learn, XGBoost（base Anaconda 環境）
...
- [ ] **Step 1: 安裝 dash 與 dash-bootstrap-components**

```powershell
& "C:\Users\user\Anaconda3\python.exe" -m pip install dash dash-bootstrap-components 2>&1 | Select-Object -Last 6
```

Expected（最後幾行）：
```
Successfully installed dash-X.X.X dash-bootstrap-components-X.X.X ...
```

- [ ] **Step 2: 驗證 import 正常**

```powershell
& "C:\Users\user\Anaconda3\python.exe" -c "import dash, dash_bootstrap_components as dbc; print('dash', dash.__version__, '/ dbc', dbc.__version__)"
```

Expected：
```
dash 2.x.x / dbc 1.x.x
```

---

### Task 2: 建立 app 骨架並確認可啟動

**Files:**
- Create: `nekoha-repo/system_dash.py`

- [ ] **Step 1: 建立骨架**

Create `nekoha-repo/system_dash.py` with:
...
app.layout = html.Div("啟動成功", style={"color": "white"})
...
```powershell
& "C:\Users\user\Anaconda3\python.exe" "C:\Users\user\OneDrive - 國立成功大學 National Cheng Kung University\成大\商管程設\project\nekoha-repo\system_dash.py"
```

開啟 `http://127.0.0.1:8050`，應在深色背景看到「啟動成功」。Ctrl+C 停止。
...
**Files:**
- Modify: `nekoha-repo/system_dash.py`

- [ ] **Step 1: 替換 system_dash.py 全文為以下內容（含全域資料）**
...
app.layout = html.Div("載入中...", style={"color": "white"})
...
import os; os.chdir(r'C:\Users\user\OneDrive - 國立成功大學 National Cheng Kung University\成大\商管程設\project\nekoha-repo')
import pandas as pd, numpy as np, joblib
from sklearn.metrics import roc_auc_score, confusion_matrix
model_pipeline = joblib.load('fraud_model_pipeline.pkl')
X_test, y_test = joblib.load('test_data.pkl')
y_proba_all = model_pipeline.predict_proba(X_test)[:, 1]
print('AUC:', round(float(roc_auc_score(y_test, y_proba_all)), 3))
...
### Task 4: 側邊欄、版面骨架與頁面 HTML 結構

**Files:**
- Modify: `nekoha-repo/system_dash.py`

- [ ] **Step 1: 在 `app = dash.Dash(...)` 之前插入版面 helpers 與三頁面函式**

Insert before `app = dash.Dash(...)`:
...
- [ ] **Step 2: 替換 `app.layout`**

Replace `app.layout = html.Div("載入中...", ...)` with:
...
Run `system_dash.py`, open `http://127.0.0.1:8050`. Should see:
- 左側深色側邊欄，含三個 NavLink 與滑桿
- 右側 4 張 KPI 卡（顯示「—」）
- 交易監控表格（空白）
- 終端機無 traceback
...
### Task 5: Page routing callback 與 scenario callback

**Files:**
- Modify: `nekoha-repo/system_dash.py`

- [ ] **Step 1: 加入兩個 callbacks，插入在 `if __name__ == "__main__":` 之前**
...
Run `system_dash.py`. 依序點選側邊欄的「分析」和「報表」連結，內容應切換。點「嚴格資安防禦」下拉選項，閥值滑桿應跳至 0.20。

---

### Task 6: 主要 dashboard callback（含啟動空白修復）

**Files:**
- Modify: `nekoha-repo/system_dash.py`

此 callback 在頁面載入時立即觸發（`prevent_initial_call=False`），修復啟動空白問題。

- [ ] **Step 1: 插入主要 callback（在 `handle_scenario` 之後）**
...
Run `system_dash.py`, open `http://127.0.0.1:8050`。頁面載入後 **不需點任何按鈕**，KPI 卡、交易表格、判定結果、利潤文字應在 1 秒內自動填入。

- [ ] **Step 3: 確認互動功能**

- 點「📥 接收即時交易」→ 表格新增一筆，判定結果更新 ✓
- 移動「風險判定閥值」滑桿 → KPI 卡與利潤文字即時更新 ✓
- 切換「嚴格資安防禦」→ 閥值跳至 0.20，KPI 卡更新 ✓
...
- [ ] **Step 1: 插入報表相關兩個 callbacks（在 `update_dashboard` 之後）**
...
Run `system_dash.py`, navigate to 報表 page:
- Click「產生管理報表」→ 今日風控摘要文字出現 ✓
- Click「⬇ 下載報表 CSV」→ 瀏覽器下載 `daily_report.csv` ✓
...
- KPI 卡在 **不點任何按鈕** 的情況下自動填入 ✓
- 點「📥 接收即時交易」多次 → 表格保持 5 筆，最新在最上方 ✓
- 移動閥值滑桿 → KPI 卡、利潤文字即時更新，SOP 狀態可能改變 ✓
- 移動 FN Cost 滑桿 → 利潤文字中「成功防堵呆帳」金額隨之變動 ✓
- 切換「嚴格資安防禦」→ 閥值自動跳至 0.20 ✓

- [ ] **Step 2: 分析頁驗證**

- 點側邊欄「📊 分析」→ 顯示特徵重要性圖（藍色橫條）與利潤曲線 ✓
- 回監控頁移動閥值，再切換回分析頁 → 利潤曲線的紅點位置應更新 ✓
- 切回監控頁，監控資料仍完整（頁面切換不重置資料）✓

- [ ] **Step 3: 報表頁驗證**

- 點側邊欄「📋 報表」→ 顯示報表頁面 ✓
- Click「產生管理報表」→ 風控摘要出現，摘要中的閥值與監控頁一致 ✓
- Click「⬇ 下載報表 CSV」→ 下載成功，CSV 以 `daily_report.csv` 命名 ✓

- [ ] **Step 4: 終端機清潔**

整個測試過程中，終端機不出現 Python traceback 或 Dash callback exception ✓
